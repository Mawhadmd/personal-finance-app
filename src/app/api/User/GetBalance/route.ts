...
export async function GET(request: NextRequest) {
    ...
    const user_id = searchParams.get("user_id")!;
     await pool.query(
      'UPDATE "User" SET balance = (SELECT COALESCE(SUM(amount), 0) FROM "Income" WHERE user_id = $1) - (SELECT COALESCE(SUM(amount), 0) FROM "Expenses" WHERE user_id = $1) WHERE user_id = $1',
      [parseInt(user_id)]
    );
